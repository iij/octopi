...
static const char* const subcmd = "list";
...
static void
list_vni(OD *od, uint32_t vni, uint8_t proto)
{
	open_rule_file(od, O_RDONLY);
	LPM_list(NULL, COMMAND_NAME, vni, proto);
	close_rule_file(od);
}

static void
list_all(OD *od, uint8_t proto)
{
	uint32_t count;
	uint64_t *list;
	uint32_t n;
...
void
command_list(OD *od, int ac, char *av[])
{
