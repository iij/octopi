...
static const char* const subcmd = "restore";

static void
command_usage(int status)
{
	info("usage: %s [rule] %s [protocol]", COMMAND_NAME, subcmd);
...
void
command_restore(OD *od, int ac, char *av[])
{
	 ...
	 *  resture
	 */
	if (update_rules(od, STDIN_FILENO, proto, TRUE, NULL) == FALSE)
		exit(EXIT_FAILURE);
}
