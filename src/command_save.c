...
static const char* const subcmd = "save";
...
static void
save_vni(OD *od, uint32_t vni, uint8_t proto)
{
	open_rule_file(od, O_RDONLY);
	LPM_save(NULL, vni, proto);
	zprintf(NULL, "end\n");
	close_rule_file(od);
}

static void
save_all(OD *od, uint8_t proto)
{
	uint32_t count;
	uint64_t *list;
	uint32_t n;
...
void
command_save(OD *od, int ac, char *av[])
{
